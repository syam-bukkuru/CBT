...
import { X, Database, Cpu, BookOpen, Pencil } from 'lucide-react'
...
export default function EditSubjectModal({ subject, onClose, onUpdate, existingNames = [] }) {
  const [name, setName] = useState(subject.name)
  const [description, setDescription] = useState(subject.description || '')
  const [icon, setIcon] = useState(subject.icon || 'Database')
  ...
  const submit = async (e) => {
    ...
    if (
      trimmed.toLowerCase() !== subject.name.toLowerCase() &&
    ...
    setError('')
    try {
      await onUpdate(subject._id, { name: trimmed, description: description.trim(), icon })
      onClose()
    } catch (err) {
      setError(err.message || 'Could not update the subject.')
      setSaving(false)
    }
  }
      ...
      <div className="modal-content" style={{ maxWidth: 480 }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-[var(--text-main)]">Edit Subject Folder</h3>
          </div>
